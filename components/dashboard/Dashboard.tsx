  ...
  const handleSendMessage = async (text: string, model: string, attachment?: { data: string, mimeType: string }) => {
    ...
    try {
      ...
      if (!currentConvId) {
        ...
        setConversations(prev => [{ id: newConvData.id, title: newConvData.title, messages: [] }, ...prev]);
      ...
      setConversations(prev => prev.map(c => c.id === currentConvId ? { ...c, messages: [...c.messages, userMsg] } : c));
      ...
      setConversations(prev => prev.map(c => c.id === currentConvId ? { ...c, messages: [...c.messages, placeholderMsg] } : c));
      ...
      if (currentConvId) {
        setConversations(prev => prev.map(c => c.id === currentConvId ? { ...c, messages: [...c.messages, errorMsg] } : c));
      }
    } finally { 
      setIsTyping(false); 
    }
