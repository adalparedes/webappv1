...
const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  }, [conversation?.messages, isTyping, showScrollDown]);
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCompressionStatus('Recibiendo archivo...');
    ...
    try {
        setCompressionStatus('Comprimiendo imagen...');
        const compressedBase64 = await compressImage(file);
        setCompressionStatus(null);
        setAttachment({ data: compressedBase64, mimeType: 'image/jpeg', name: file.name });
    } catch (error) {
        console.error("Fallo de compresión:", error);
        setCompressionStatus('Error al procesar');
        setTimeout(() => {
            setCompressionStatus(null);
            setAttachment(null);
        }, 3000);
  ...
  const isProcessingAttachment = isSendingAttachment && isTyping;
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-[#050505] h-full relative">
      <div 
        ...
        className={`flex-1 min-h-0 overflow-y-auto p-4 lg:p-8 space-y-8 custom-scrollbar scroll-smooth transition-all duration-300 ${isBottomNavOpen ? 'pb-88' : 'pb-64'} lg:pb-40`}
      >
        {isLoadingHistory ? (
      ...
      {showScrollDown && (
        <button
          ...
          className={`absolute z-30 right-6 lg:right-10 p-3 bg-[#00d2ff] text-black rounded-full shadow-lg hover:scale-110 active:scale-95 transition-all animate-bounce
                      lg:bottom-44 
                      ${isBottomNavOpen ? 'bottom-64' : 'bottom-40'}`}
      ...
      <div className={`absolute left-0 w-full p-4 lg:p-6 bg-transparent z-40 lg:relative lg:bottom-auto transition-all duration-300 ${isBottomNavOpen ? 'bottom-48' : 'bottom-28'}`}>
        <div className="max-w-4xl mx-auto space-y-3">
          <div className="flex items-center justify-between px-2">
             ...
             {(attachment || compressionStatus || isProcessingAttachment) && (
                <div className="flex items-center gap-2 px-3 py-1 bg-[#00d2ff]/10 border border-[#00d2ff]/30 rounded-full animate-in fade-in slide-in-from-bottom-2">
                  {isProcessingAttachment ? (
                    <>
                      ...
                      <span className="text-[9px] font-mono text-white truncate max-w-[200px]">Enviando y procesando IA...</span>
                    </>
                  ) : compressionStatus ? (
                    <>
                      <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                      <span className="text-[9px] font-mono text-white truncate max-w-[150px]">{compressionStatus}</span>
                    </>
                  ) : attachment ? (
                    <>
                      ...
                      <button onClick={() => { setAttachment(null); setCompressionStatus(null); }} className="text-[10px] text-red-400 hover:text-white ml-1">×</button>
                    </>
                  ) : null}
                </div>
             )}
